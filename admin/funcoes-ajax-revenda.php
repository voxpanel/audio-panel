...
if($acao == "ligar_streaming") {
	...
	$config_streaming .= "streamintrofile_1=/home/streaming/".$dados_stm["porta"]."/streaming_intro.mp3\n";
	}
	
	if($dados_stm["arquivo_backup"]) {
	$config_streaming .= "streambackupfile_1=/home/streaming/".$dados_stm["porta"]."/streaming_backup.mp3\n";
	}
	...
	echo "<span class='texto_status_sucesso'>Streaming ".$dados_stm["porta"]." ligado com sucesso.</span><br /><br/><a href='javascript:void(0);' onClick='status_streaming(\"".code_decode($dados_stm["porta"],"E")."\");document.getElementById(\"log-sistema-fundo\").style.display = \"none\";document.getElementById(\"log-sistema\").style.display = \"none\";' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
// Função para ligar streamings da revenda em massa
if($acao == "ligar_streamings_revenda") {

	
	$sql_lista_stms = mysql_query("SELECT * FROM streamings where status = '1' AND codigo_cliente = '".$_SESSION["code_user_logged"]."' ORDER by porta ASC");
	while ($dados_stm = mysql_fetch_array($sql_lista_stms)) {
	...
	$resultado_acao = $ssh->executar("/home/streaming/ligar_streaming /home/streaming/configs/".$config_streaming."");
	
	$resultado_acao = str_replace("\n","",$resultado_acao);
	
	if(is_numeric($resultado_acao)) {
	
	$status_streaming = status_streaming($dados_servidor["ip"],$dados_stm["porta"]);
	
	if($status_streaming == "ligado") {
	
	$resultado .= "<span class='texto_status_sucesso'>".sprintf(lang_acao_ligar_stm_massa_resultado_ok, $dados_stm["porta"])."</span><br>";
	...
	$resultado .= "<span class='texto_status_erro'>".sprintf(lang_acao_ligar_stm_massa_resultado_erro, $dados_stm["porta"])."</span><br>";
	...
	$resultado .= "<br><span class='texto_status_sucesso'>".lang_acao_ligar_stm_massa_resultado_fim."</span><br>";
...
if($acao == "bloquear_streaming") {
	...
	echo "<span class='texto_status_sucesso'>Streaming ".$dados_stm["porta"]." bloqueado com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "desbloquear_streaming") {
	...
	echo "<span class='texto_status_sucesso'>Streaming ".$dados_stm["porta"]." desbloqueado com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "remover_streaming") {
	...
	echo "<span class='texto_status_sucesso'>Streaming ".$dados_stm["porta"]." removido com sucesso.</span><br /><br/><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "status_streaming") {
	...
	echo "<font color=\"#009900\" size=\"5\"><strong>".lang_info_status_transmitindo."</strong></font><br><font color=\"#009900\" size=\"2\"><strong>".lang_info_status_relay."</strong></font>";
	exit();
	}
	
	if($status_conexao_transmissao == "autodj") {
	echo "<font color=\"#009900\" size=\"5\"><strong>".lang_info_status_transmitindo."</strong></font><br><font color=\"#009900\" size=\"2\"><strong>".lang_info_status_autodj."</strong></font>";
	exit();
	}
...
// Função para verificar o status do streaming e autodj
if($acao == "status_streaming_busca_avancada") {
	...
	if($status_conexao_transmissao == "relay") {
	echo lang_info_status_relay."|#A8FFA8";
	exit();
	}
	
	if($status_conexao_transmissao == "autodj") {
	echo lang_info_status_autodj."|#A8FFA8";
	exit();
	}
...
if($acao == "acessar_painel_streaming_revenda") {
	...
	echo "1|".code_decode($dados_stm["porta"],"E")."@".code_decode($dados_stm["senha"],"E")."";
	
	// Insere a ação executada no registro de logs.
	logar_acao("[".$dados_stm["porta"]."] Acesso da revenda ao painel de streaming executado com sucesso.");
...
// Função para ligar autodj
if($acao == "ligar_autodjs_revenda") {

	
	$sql_lista_stms = mysql_query("SELECT * FROM streamings where status = '1' AND autodj = 'sim' AND codigo_cliente = '".$_SESSION["code_user_logged"]."' ORDER by porta ASC");
	while ($dados_stm = mysql_fetch_array($sql_lista_stms)) {
	
	if($dados_stm["relay"] == "nao") {
	...
	$config_autodj .= "streamurl=".$dados_stm["streamurl"]."\n";
	$config_autodj .= "genre=".$dados_stm["genre"]."\n";
	$config_autodj .= "shuffle=".$dados_stm["autodj_shuffle"]."\n";
	$config_autodj .= "xfade=".$dados_stm["xfade"]."\n";
	$config_autodj .= "xfadethreshold=20\n";
	$config_autodj .= "flashpolicyfile=/home/streaming/configs/crossdomain.xml\n\n";
	$config_autodj .= ";DJ PORT CONFIG\n";
	$config_autodj .= "calendarfile=/home/streaming/configs/calendar-".$dados_stm["porta"].".xml\n";
	...
	$resultado_acao = $ssh->executar("/home/streaming/ligar_autodj /home/streaming/configs/".$config_autodj."");
	
	$resultado_acao = str_replace("\n","",$resultado_acao);
	
	if(is_numeric($resultado_acao) || $resultado_acao == "ok") {
	
	$resultado .= "<span class='texto_status_sucesso'>".sprintf(lang_acao_ligar_autodj_massa_resultado_ok, $dados_stm["porta"])."</span><br>";
	...
	$resultado .= "<span class='texto_status_erro'>".sprintf(lang_acao_ligar_autodj_massa_resultado_erro, $dados_stm["porta"])."</span><br>";
	...
	$resultado .= "<span class='texto_status_alerta'>".sprintf(lang_acao_ligar_autodj_massa_resultado_relay, $dados_stm["porta"])."</span><br>";
	
	} // -> status relay
		
	} // -> while
	
	$resultado .= "<br><span class='texto_status_sucesso'>".lang_acao_ligar_autodj_massa_resultado_fim."</span><br>";
...
if($acao == "bloquear_revenda") {
	...
	mysql_query("Update revendas set status = '2' where status = '1' AND codigo = '".$dados_revenda["codigo"]."'");
	
	echo "<span class='texto_status_sucesso'><br />Revenda <strong>".$dados_revenda["nome"]."</strong> bloqueada com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "desbloquear_revenda") {
	...
	echo "<span class='texto_status_sucesso'><br />Revenda <strong>".$dados_revenda["nome"]."</strong> desbloqueada com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "remover_revenda") {
	...
	echo "<span class='texto_status_sucesso'>Revenda <strong>".$dados_revenda["nome"]."</strong> removida com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "alterar_senha_revenda") {
	...
	echo "<span class='texto_status_sucesso'>Senha da revenda <strong>".$dados_revenda["nome"]."</strong> alterada com sucesso para ".$nova_senha."</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "alterar_senha_streaming") {
	...
	echo "<span class='texto_status_sucesso'>Senha do streaming <strong>".$dados_stm["porta"]."</strong> alterada com sucesso para: ".$nova_senha."</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "remover_app_android") {
	...
	echo "<span class='texto_status_sucesso'>Requisição do streaming <strong>".$dados_stm["porta"]."</strong> removido com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "bloquear_subrevenda") {
	...
	$dados_subrevenda = mysql_fetch_array(mysql_query("SELECT * FROM revendas where codigo = '".$codigo."'"));
	...
	mysql_query("Update streamings set status = '2' where status = '1' AND codigo = '".$dados_stm["codigo"]."'");
	...
	echo "<span class='texto_status_sucesso'><br />Revenda <strong>".$dados_subrevenda["nome"]."</strong> bloqueada com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "desbloquear_subrevenda") {
	...
	if($codigo == "" || $codigo == 0) {
	die("<span class='texto_status_erro'>".lang_alerta_dados_faltando."</span>");
	}
	...
	mysql_query("Update revendas set status = '1' where codigo_revenda = '".$dados_subrevenda["codigo"]."' AND tipo = '3'");
	
	// Desbloqueia as subrevendas da subrevenda
	$query_subrevendas_sub = mysql_query("SELECT * FROM revendas where codigo_revenda = '".$dados_subrevenda["codigo"]."' AND tipo = '3' ORDER by codigo ASC");
	while ($dados_subrevenda_sub = mysql_fetch_array($query_subrevendas_sub)) {
	...
	echo "<span class='texto_status_sucesso'><br />Revenda <strong>".$dados_subrevenda["nome"]."</strong> desbloqueada com sucesso.</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
...
if($acao == "remover_subrevenda") {
	...
	$checar_subrevenda = mysql_num_rows(mysql_query("SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".$codigo."') AND tipo != '1'"));
	...
	// Remove os streamings da subrevenda
	$query_stms = mysql_query("SELECT * FROM streamings where codigo_cliente = '".$dados_subrevenda["codigo"]."'");
	while ($dados_stm = mysql_fetch_array($query_stms)) {
	...
	// Insere a ação executada no registro de logs.
	logar_acao("[".$dados_stm["porta"]."] Streaming removido com sucesso na remoção da sub revenda ".$dados_subrevenda["id"]." - ".$dados_subrevenda["email"]."");
	...
	// Insere a ação executada no registro de logs.
	logar_acao("[".$dados_stm_sub["porta"]."] Streaming removido com sucesso na remoção da sub revenda ".$dados_subrevenda_sub["id"]." - ".$dados_subrevenda_sub["email"]."");
	...
	// Remove a subrevenda do painel
	mysql_query("Delete From revendas where codigo = '".$dados_subrevenda["codigo"]."'");
	
	echo "<span class='texto_status_sucesso'>Revenda <strong>".$dados_subrevenda["id"]."</strong> removida com sucesso.</span><br /><br /><a href='/admin/revenda' class='texto_status_atualizar'>[".lang_botao_titulo_fechar."]</a>";
...
if($acao == "mover_streaming_subrevenda_subrevenda") {
	...
	$verifica_subrevenda_atual = mysql_num_rows(mysql_query("SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".$dados_stm["codigo_cliente"]."') AND tipo = '2'"));
	
	$verifica_subrevenda_nova = mysql_num_rows(mysql_query("SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND id = '".$id."') AND tipo = '2'"));
	
	// Verifica se o streaming é de uma sub revenda que pertence a revenda logada
	if($verifica_subrevenda_atual > 0 && $verifica_subrevenda_nova > 0) {
	
	$dados_subrevenda = mysql_fetch_array(mysql_query("SELECT * FROM revendas WHERE id = '".$id."'"));

	mysql_query("Update streamings set codigo_cliente = '".$dados_subrevenda["codigo"]."' where codigo = '".$dados_stm["codigo"]."'");
	
	echo "<span class='texto_status_sucesso'>".lang_info_subrevenda_streamings_mover_streaming_resultado_ok."</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
	
	// Insere a ação executada no registro de logs.
	logar_acao("[".$dados_stm["porta"]."] Movido para sub revenda ".$dados_subrevenda["id"]."");
...
// Função para mover um streaming de uma revenda para uma sub revenda
if($acao == "mover_streaming_revenda_subrevenda") {
	...
	// Verifica se o streaming é da revenda atual
	$verifica_streaming = mysql_num_rows(mysql_query("SELECT * FROM streamings WHERE codigo_cliente = '".$dados_revenda["codigo"]."' AND codigo = '".$dados_stm["codigo"]."'"));
	
	// Verifica se o streaming é de uma sub revenda que pertence a revenda logada
	if($verifica_streaming > 0) {
	...
	// Insere a ação executada no registro de logs.
	logar_acao("[".$dados_stm["porta"]."] Movido da revenda para sub revenda ".$dados_subrevenda["id"]."");
...
if($acao == "buscar_subrevenda") {
	...
	$verifica_subrevenda = mysql_num_rows(mysql_query("SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND id = '".query_string('3')."') AND tipo = '2'"));
	
	if($verifica_subrevenda > 0) {
	
	$dados_subrevenda = mysql_fetch_array(mysql_query("SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND id = '".query_string('3')."') AND tipo = '2'"));
...
// Função para ligar todos os streamings da revenda
if($acao == "listar_streamings_revenda") {


	
	$sql = mysql_query("SELECT * FROM streamings where status = '1' AND codigo_cliente = '".$_SESSION["code_user_logged"]."' ORDER by porta ASC");
...
// Função para ligar todos os autodjs da revenda
if($acao == "listar_autodjs_revenda") {
	...
	$sql = mysql_query("SELECT * FROM streamings where status = '1' AND codigo_cliente = '".$_SESSION["code_user_logged"]."' ORDER by porta ASC LIMIT 1");
	while ($dados_stm = mysql_fetch_array($sql)) {
