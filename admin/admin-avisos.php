...
$sql = mysql_query("SELECT * FROM avisos");
$lpp = 100; // total de registros por p&aacute;gina
$total = mysql_num_rows($sql);
$paginas = ceil($total / $lpp); 
if(!isset($pagina_atual)) { $pagina_atual = 0; }
$inicio = $pagina_atual * $lpp;
$sql = mysql_query("SELECT *, DATE_FORMAT(data,'%d/%m/%Y') AS data FROM avisos ORDER by codigo ASC LIMIT $inicio, $lpp");

while ($dados_aviso = mysql_fetch_array($sql)) {

$total_vizualizacoes = mysql_num_rows(mysql_query("SELECT * FROM avisos_desativados where codigo_aviso = '".$dados_aviso["codigo"]."'"));
...
$total_registros = mysql_num_rows(mysql_query("SELECT * FROM avisos"));
