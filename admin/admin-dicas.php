...
<?php
$sql = mysql_query("SELECT * FROM dicas_rapidas");
$lpp = 100; // total de registros por p&aacute;gina
$total = mysql_num_rows($sql);
$paginas = ceil($total / $lpp); 
if(!isset($pagina_atual)) { $pagina_atual = 0; }
$inicio = $pagina_atual * $lpp;
$sql = mysql_query("SELECT * FROM dicas_rapidas ORDER by codigo ASC LIMIT $inicio, $lpp");

while ($dados_dica_rapida = mysql_fetch_array($sql)) {

$total_vizualizacoes = mysql_num_rows(mysql_query("SELECT * FROM dicas_rapidas_acessos where codigo_dica = '".$dados_dica_rapida["codigo"]."'"));
...
$total_registros = mysql_num_rows(mysql_query("SELECT * FROM dicas_rapidas"));
