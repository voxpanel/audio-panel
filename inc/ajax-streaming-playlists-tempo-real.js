...
function musica_atual_proxima_musica( porta, caracteres ) {
	
  if(porta == "") {
  alert("Error!\n\nPortuguês: Dados faltando, tente novamente ou contate o suporte.\n\nEnglish: Missing data try again or contact support.\n\nEspañol: Los datos que faltaban inténtelo de nuevo o contacte con Atención.");
  } else {
  
  document.getElementById("musica_atual").innerHTML = "<img src='http://"+get_host()+"/img/spinner.gif' />";
  document.getElementById("proxima_musica").innerHTML = "<img src='http://"+get_host()+"/img/spinner.gif' />";
  
  var http = new Ajax();
  http.open("GET", "/funcoes-ajax/musica_atual_proxima_musica/"+porta+"/"+caracteres , true);
  http.onreadystatechange = function() {
  ...
  if(http.readyState == 4) {
	...
	document.getElementById("proxima_musica").innerHTML = dados[1];	
